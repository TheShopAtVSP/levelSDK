...
class Frame: DataPacket {
    ...
    convenience init(bytes: [UInt8]) {
        ...
        let modelNum = Int(BitsHelper.convertToUInt16(bytes[3], lsb: bytes[2]))
        let colorNum = Int(BitsHelper.convertToUInt16(bytes[5], lsb: bytes[4]))
    ...
    override func getPacket() -> [UInt8] {
        ...
        var modelBytes: [UInt8] = BitsHelper.convertTo2Bytes(model.rawValue)
        var colorBytes: [UInt8] = BitsHelper.convertTo2Bytes(color.rawValue)
